...
class Computer < Player
  ...
  def inititialize_losing_move_counters
  ...
  def losing_move_counters
    losing_move_counters = inititialize_losing_move_counters
    (0...history.size).each do |i|
      if history[i][RESULT] == "human"
        losing_move_counters[history[i][CMOVE]] += 1
      end
    end
    losing_move_counters
  end
  ...
  def calc_weights(losing_move_counters, total)
    weights = {}
  ...
  def determine_weighed_move(losing_move_counters)
    counter_total = losing_move_counters.values.reduce(:+)
    weights = calc_weights(losing_move_counters, counter_total)
    weight_total = weights.values.reduce(:+)
    random_value = rand(1..weight_total)
    sum_of_weights = 0
  ...
  def next_move
    ...
    bad_move = ''
    losing_move_counters = update_losing_move_counters
    biggest_losing_move_total = losing_move_counters.values.max
    return rand(1..5) if biggest_losing_move_total == 0
    losing_move_counters.each do |key, value|
      bad_move = key if value == biggest_losing_move_total
    end
    select_best_move_for(bad_move, losing_move_counters)
  end
...
class RPSGame
  ...
  MAX_SCORE = 10
  attr_accessor :human, :computer, :token
